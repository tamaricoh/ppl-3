...
export const makePrimOp = (op: PrimOpKeyword): PrimOp => ({
  tag: "PrimOp",
  op: op,
});
...
export const makeVarDecl = (v: string, te: TExp): VarDecl => ({
  tag: "VarDecl",
  var: v,
  texp: te,
});
...
export type ProcExp = {
  ...
  args: VarDecl[];
  body: CExp[];
  returnTE: TExp | typePredTExp;
};
export const makeProcExp = (
  args: VarDecl[],
  body: CExp[],
  returnTE: TExp | typePredTExp
): ProcExp => ({ tag: "ProcExp", args: args, body: body, returnTE: returnTE });
...
export const makeLetrecExp = (
  bindings: Binding[],
  body: CExp[]
): LetrecExp => ({ tag: "LetrecExp", bindings: bindings, body: body });
...
export type SpecialFormKeyword =
  | "lambda"
  | "let"
  | "letrec"
  | "if"
...
export const parseL5Program = (sexp: Sexp): Result<Program> =>
  isToken(sexp)
    ...
    ? parseL5GoodProgram(first(sexp), rest(sexp))
    : makeFailure("Unexpected empty program");

const parseL5GoodProgram = (keyword: Sexp, body: Sexp[]): Result<Program> =>
  keyword === "L5" && !isEmpty(body)
    ? mapv(mapResult(parseL5Exp, body), (exps: Exp[]) => makeProgram(exps))
    : makeFailure(
        `Program must be of the form (L5 <exp>+): ${format([keyword, ...body])}`
      );

export const parseL5Exp = (sexp: Sexp): Result<Exp> =>
  isNonEmptyList<Sexp>(sexp)
    ? parseL5CompoundExp(first(sexp), rest(sexp))
    : isToken(sexp)
    ? parseL5Atomic(sexp)
    : makeFailure("Exp cannot be an empty list");

export const parseL5CompoundExp = (op: Sexp, params: Sexp[]): Result<Exp> =>
  op === "define" ? parseDefine(params) : parseL5CompoundCExp(op, params);

export const parseL5CompoundCExp = (op: Sexp, params: Sexp[]): Result<CExp> =>
  isString(op) && isSpecialFormKeyword(op)
    ? parseL5SpecialForm(op, params)
    : parseAppExp(op, params);

export const parseL5SpecialForm = (
  op: SpecialFormKeyword,
  params: Sexp[]
): Result<CExp> =>
...
export const parseL5CExp = (sexp: Sexp): Result<CExp> =>
  isNonEmptyList<Sexp>(sexp)
    ? parseL5CompoundCExp(first(sexp), rest(sexp))
    : isToken(sexp)
    ? parseL5Atomic(sexp)
...
const parseProcExp = (vars: Sexp, rest: Sexp[]): Result<ProcExp> => {
  if (isArray(vars)) {
    if (vars.length === 3 && rest[0] === "is") {
      return parseTypePred(vars, rest);
    }
    ...
    return bind(args, (args: VarDecl[]) =>
      bind(body, (body: CExp[]) =>
        mapv(returnTE, (returnTE: TExp) => makeProcExp(args, body, returnTE))
      )
    );
  } else {
...
const parseTypePred = (vars: Sexp[], rest: Sexp[]): Result<ProcExp> => {
  if (vars.length === 2 && rest[0] === "is") {
    ...
    return makeFailure("Invalid type predicate syntax");
  }
};
...
const parseLetExp = (bindings: Sexp, body: Sexp[]): Result<LetExp> =>
  isEmpty(body)
    ? makeFailure('Body of "let" cannot be empty')
    : !isGoodBindings(bindings)
    ? makeFailure(`Invalid bindings: ${format(bindings)}`)
  ...
  bind(
    ...
    (vds: VarDecl[]) =>
      mapv(
        ...
        (vals: CExp[]) => zipWith(makeBinding, vds, vals)
      )
  );

const parseLetrecExp = (bindings: Sexp, body: Sexp[]): Result<LetrecExp> =>
  isEmpty(body)
    ? makeFailure('Body of "letrec" cannot be empty')
    : !isGoodBindings(bindings)
    ? makeFailure(`Invalid bindings: ${format(bindings)}`)
    : bind(parseBindings(bindings), (bdgs: Binding[]) =>
        mapv(mapResult(parseL5CExp, body), (body: CExp[]) =>
...
const parseGoodSetExp = (variable: Sexp, val: Sexp): Result<SetExp> =>
  !isIdentifier(variable)
    ? makeFailure(
        `First arg of set! must be an identifier: ${format(variable)}`
      )
    : mapv(parseL5CExp(val), (val: CExp) =>
        makeSetExp(makeVarRef(variable), val)
      );
...
export const parseSExp = (sexp: Sexp): Result<SExpValue> =>
  sexp === "#t"
    ...
    : isArray(sexp) && isDottedPair(sexp)
    ? makeDottedPair(sexp)
    : isNonEmptyList<Sexp>(sexp)
    ? // fail on (x . y z)
      sexp[0] === "."
...
export const unparse = (e: Parsed): Result<string> =>
  ...
  isNumExp(e)
    ...
    isAppExp(e)
    ? bind(unparse(e.rator), (rator: string) =>
        mapv(
          mapResult(unparse, e.rands),
          (rands: string[]) => `(${rator} ${join(" ", rands)})`
        )
      )
    : isIfExp(e)
    ? bind(unparse(e.test), (test: string) =>
        bind(unparse(e.then), (then: string) =>
          mapv(unparse(e.alt), (alt: string) => `(if ${test} ${then} ${alt})`)
        )
      )
    : isLetExp(e)
    ? unparseLetExp(e)
    ...
    ? unparseProcExp(e)
    : isLitExp(e)
    ? makeOk(unparseLitExp(e))
    : isSetExp(e)
    ? unparseSetExp(e)
    : // DefineExp | Program
    isDefineExp(e)
    ? bind(unparseVarDecl(e.var), (vd: string) =>
        mapv(unparse(e.val), (val: string) => `(define ${vd} ${val})`)
      )
    : isProgram(e)
    ? mapv(unparseLExps(e.exps), (exps: string) => `(L5 ${exps})`)
    : e;

const unparseReturn = (te: TExp): Result<string> =>
  isTVar(te) ? makeOk("") : mapv(unparseTExp(te), (te: string) => ` : ${te}`);

const unparseBindings = (bindings: Binding[]): Result<string> =>
  mapv(
    mapResult(
      (bdg) =>
        bind(unparseVarDecl(bdg.var), (vd: string) =>
          mapv(unparse(bdg.val), (val: string) => `(${vd} ${val})`)
        ),
      bindings
    ),
    (bdgs: string[]) => join(" ", bdgs)
  );
  ...
  bind(mapResult(unparseVarDecl, pe.args), (vds: string[]) =>
    bind(unparseReturn(pe.returnTE), (ret: string) =>
      mapv(
        ...
        (body: string) => `(lambda (${join(" ", vds)})${ret} ${body})`
      )
    )
  );
...
const unparseLetExp = (le: LetExp): Result<string> =>
  bind(unparseBindings(le.bindings), (bdgs: string) =>
    mapv(unparseLExps(le.body), (body: string) => `(let (${bdgs}) ${body})`)
  );

const unparseLetrecExp = (le: LetrecExp): Result<string> =>
  bind(unparseBindings(le.bindings), (bdgs: string) =>
    mapv(unparseLExps(le.body), (body: string) => `(letrec (${bdgs}) ${body})`)
  );
