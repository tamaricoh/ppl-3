...
export const typeofDefine = (exp: DefineExp, tenv: TEnv): Result<VoidTExp> =>
  bind(
    typeofExp(exp.val, makeExtendTEnv([exp.var.var], [exp.var.texp], tenv)),
...
export const typeofSequence = (seq: List<Exp>, tenv: TEnv): Result<TExp> =>
  isNonEmptyList<Exp>(seq)
    ? typeofSequenceFirst(first(seq), rest(seq), tenv)
...
const typeofSequenceFirst = (
  ...
  isEmpty(rest)
    ? typeofExp(first, tenv)
    : isDefineExp(first)
    ? typeofDefineExps(first, rest, tenv)
    : // _ is a don't care parameter
      bind(typeofExp(first, tenv), (_) => typeofSequence(rest, tenv));
