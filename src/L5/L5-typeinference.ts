...
const checkEqualType = (
  te1: T.TExp | undefined,
  te2: T.TExp,
  exp: A.Exp
): Result<true> =>
  te1 === undefined
    ? bind(T.unparseTExp(te2), (texp: string) =>
        makeFailure(`Incompatible types: undefined - ${format(texp)}`)
      )
    : T.isTVar(te1) && T.isTVar(te2)
    ? T.eqTVar(te1, te2)
      ? makeOk(true)
      : checkTVarEqualTypes(te1, te2, exp)
    : T.isTVar(te1)
    ? checkTVarEqualTypes(te1, te2, exp)
    : T.isTVar(te2)
    ? checkTVarEqualTypes(te2, te1, exp)
    : T.isAtomicTExp(te1) && T.isAtomicTExp(te2)
    ? T.eqAtomicTExp(te1, te2)
      ? makeOk(true)
    ...
    : T.isProcTExp(te1) && T.isProcTExp(te2)
    ? checkProcEqualTypes(te1, te2, exp)
    : bind(T.unparseTExp(te1), (te1: string) =>
        bind(T.unparseTExp(te2), (te2: string) =>
...
// Exp is only passed for documentation purposes.
const checkEqualTypes = (
  tes1: T.TExp[],
  tes2: T.TExp[],
  exp: A.Exp
): Result<true> => {
  const checks = zipWithResult(
    (te1, te2) => checkEqualType(te1, te2, exp),
    tes1,
    tes2
  );
...
const checkProcEqualTypes = (
  ...
  te1.paramTEs.length !== te2.paramTEs.length
    ...
    : checkEqualTypes(
        T.procTExpComponents(te1),
        T.procTExpComponents(te2),
        exp
      );
...
// Exp is only passed for documentation purposes.
const checkTVarEqualTypes = (
  tvar: T.TVar,
  te: T.TExp,
  exp: A.Exp
): Result<true> =>
  T.tvarIsNonEmpty(tvar)
    ? checkEqualType(T.tvarContents(tvar), te, exp)
...
const checkNoOccurrence = (
  ...
  const loop = (te1: T.TExp): Result<true> =>
    T.isAtomicTExp(te1)
      ? makeOk(true)
      : T.isProcTExp(te1)
      ? checkList(T.procTExpComponents(te1))
      : T.isTVar(te1)
      ? T.eqTVar(te1, tvar)
        ? bind(A.unparse(exp), (exp: string) =>
            makeFailure(
              `Occur check error - ${te1.var} - ${tvar.var} in ${format(exp)}`
            )
          )
        : makeOk(true)
      : bind(A.unparse(exp), (exp: string) =>
...
export const typeofExp = (exp: A.Parsed, tenv: E.TEnv): Result<T.TExp> =>
  A.isNumExp(exp)
    ? makeOk(T.makeNumTExp())
    : A.isBoolExp(exp)
    ? makeOk(T.makeBoolTExp())
    : A.isStrExp(exp)
    ? makeOk(T.makeStrTExp())
    : A.isPrimOp(exp)
    ? TC.typeofPrim(exp)
    : A.isVarRef(exp)
    ? E.applyTEnv(tenv, exp.var)
    : A.isIfExp(exp)
    ? typeofIf(exp, tenv)
    : A.isProcExp(exp)
    ? typeofProc(exp, tenv)
...
const typeofIf = (ifExp: A.IfExp, tenv: E.TEnv): Result<T.TExp> => {
  ...
  const constraint1 = bind(testTE, (testTE: T.TExp) =>
    checkEqualType(testTE, T.makeBoolTExp(), ifExp)
  );
  const constraint2 = bind(thenTE, (thenTE: T.TExp) =>
    bind(altTE, (altTE: T.TExp) => checkEqualType(thenTE, altTE, ifExp))
  );
...
export const typeofProc = (proc: A.ProcExp, tenv: E.TEnv): Result<T.TExp> => {
  ...
  const extTEnv = E.makeExtendTEnv(
    R.map((vd) => vd.var, proc.args),
    argsTEs,
    tenv
  );
  const constraint1 = bind(typeofExps(proc.body, extTEnv), (bodyTE: T.TExp) =>
    checkEqualType(bodyTE, proc.returnTE, proc)
  );
...
export const typeofLet = (exp: A.LetExp, tenv: E.TEnv): Result<T.TExp> => {
  ...
  const constraints = zipWithResult(
    (varTE, val) =>
      bind(typeofExp(val, tenv), (valTE: T.TExp) =>
  ...
  return bind(constraints, (_) =>
    typeofExps(exp.body, E.makeExtendTEnv(vars, varTEs, tenv))
  );
};
...
export const typeofLetrec = (
  ...
  const tenvBody = E.makeExtendTEnv(
    ps,
    R.zipWith((tij, ti) => T.makeProcTExp(tij, ti), tijs, tis),
    tenv
  );
  const tenvIs = R.zipWith(
    (params, tij) =>
      E.makeExtendTEnv(
        R.map((p) => p.var, params),
        tij,
        tenvBody
      ),
    paramss,
  ...
  const constraints = bind(types, (types: T.TExp[]) =>
